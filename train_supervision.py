...
def get_args():
    ...
    arg("-c", "--config_path", type=Path, help="Path to the config.", default=r'config/vaihingen/config.py')
...
class Supervision_Train(pl.LightningModule):
    ...
    def forward(self, x, ir, dsm):
        # only net is used in the prediction/inference
        seg_pre = self.net(x, ir, dsm)
        return seg_pre

    def training_step(self, batch, batch_idx):
        img, ir, dsm, mask = batch['img'], batch['ir'], batch['dsm'], batch['gt_semantic_seg']
        prediction = self.net(img, ir, dsm)
    ...
    def validation_step(self, batch, batch_idx):
        img, ir, dsm, mask = batch['img'], batch['ir'], batch['dsm'], batch['gt_semantic_seg']
        prediction = self.forward(img, ir, dsm) 
        pre_mask = nn.Softmax(dim=1)(prediction)
        pre_mask = pre_mask.argmax(dim=1)
        for i in range(mask.shape[0]):
